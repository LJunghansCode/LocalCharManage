app.controller('sessionController', ['$location', '$scope', '$route','$routeParams', 'campaignFactory','playerFactory','userFactory',function(loc, scope, route, routeParams, campaignFactory, playerFactory, userFactory){
    ...
    scope.sortPlayers = (players) => {
        ...
        let currHighest;
        for(let j=0; j<4; j++){
            sortedPlayers.push(players[currHighest]);
            players.splice(currHighest, 1);
            currHighest = 0;
            for(let i=0; i<players.length; i++){
                let init = parseInt(players[i].initiative);
                if (init > players[highestPos].initiative){
                    highestPos = i;
                }
            }
        }
